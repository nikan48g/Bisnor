...
class AuthManager(private val context:Context) {
 ...
 suspend fun register(name:String,email:String,password:String):Pair<Boolean,String> { if(!name.matches(Regex("[a-z0-9_.-]{3,32}")))return false to "نام کاربری نامعتبر است."; if(!android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()||password.length<8)return false to "ایمیل معتبر و رمز حداقل ۸ کاراکتری لازم است."; val r=SupabaseManager.signup(context,name,email,password,userAvatarId); if(r.first&&SupabaseManager.hasSession(context)){currentUsername=name;syncUp(context)};return r }
 suspend fun login(email:String,password:String):Pair<Boolean,String>{val r=SupabaseManager.login(context,email,password);if(!r.first)return r;val x=SupabaseManager.profile(context)?:return false to "پروفایل امن پیدا نشد.";currentUsername=x.optString("username");userAvatarId=x.optString("avatar_id",userAvatarId);syncDown(context,x);return true to "ورود امن انجام شد."}

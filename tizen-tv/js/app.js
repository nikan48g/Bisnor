...
class BisnorApp {
    ...
    createMediaCard(media) {
        ...
        // div cards are not focusable by default.  Samsung remotes navigate the focused
        // element, so without this a physical TV cannot reliably select any movie.
        card.tabIndex = 0;
    ...
    async renderHomeTab() {
        ...
        // The bundled seed was only an old snapshot and has known mismatched titles/details.
        // Never show it as live content: wait for the catalog provider instead.
        ...
        // Fetch fresh live data in parallel.  Rows remain empty rather than displaying
        // incorrect offline records if the provider is unavailable.
        Promise.allSettled([
    ...
    playStream(url, title) {
        ...
        // 1. If running on Samsung Smart TV (Tizen), prioritize hardware AVPlay for MKV / HEVC / Dolby
        if (window.TizenAVPlayEngine && window.TizenAVPlayEngine.isTizen()) {
            ...
            if (video) video.style.display = "none"; // Hide standard video element on TV

            const started = window.TizenAVPlayEngine.play(url, title, (cur, dur) => {
                const seekbar = document.getElementById("player-seekbar");
                const timeLabel = document.getElementById("player-time-label");
                if (seekbar && dur > 0) seekbar.value = (cur / dur) * 100;
                if (timeLabel) timeLabel.textContent = `${this.formatTime(cur)} / ${this.formatTime(dur)}`;
            }, () => {
                this.closePlayerModal();
            }, (err) => {
                console.warn("[Tizen] AVPlay error, falling back to HTML5:", err);
                if (video) {
    ...
    closePlayerModal() {
        if (window.TizenAVPlayEngine) {
            window.TizenAVPlayEngine.stop();
        }
        const modal = document.getElementById("modal-player");
        const video = document.getElementById("html-video-player");
        const errOverlay = document.getElementById("player-error-overlay");
